"""
This script processes raw game data to generate comprehensive statistics for the web application.

It performs the following key functions:
- Loads all seasonal game data from the data loader.
- Calculates detailed hitting and pitching statistics for players and teams.
- Generates advanced metrics like OPS+, ERA-, FIP, WAR, and RE24.
- Handles data corrections, including for pinch runners and multi-steal events.
- Creates run expectancy matrices for each season.
- Reconciles player IDs across different seasons.
- Exports all processed data into JSON files for the web application.
"""
...
def _read_cache_manifest(cache_dir):
    """
    Reads the cache manifest file to get the last run's most recent season.

    Args:
        cache_dir (str): The path to the cache directory.

    Returns:
        str or None: The most recent season from the last run, or None if the
...
def _write_cache_manifest(cache_dir, most_recent_season):
    """
    Writes the most recent season to the cache manifest file.

    Args:
        cache_dir (str): The path to the cache directory.
        most_recent_season (str): The most recent season to write to the manifest.
    """
    manifest_path = os.path.join(cache_dir, "cache_info.json")
    try:
...
def calculate_ops_plus_for_row(row, league_stats_by_season):
    """
    Calculates OPS+ for a single row (player season).

    Args:
        row (pd.Series): A row from a DataFrame containing player stats.
        league_stats_by_season (dict): A dictionary with league-wide stats for each season.

    Returns:
        int or pd.NA: The calculated OPS+, or pd.NA if it cannot be calculated.
    """
    if row["PA"] == 0:
        return pd.NA
    ...
    ops_plus = 100 * (
        (player_nobp / season_stats["lg_nOBP"])
        + (player_nslg / season_stats["lg_nSLG"])
        - 1
    )
...
def format_ip(ip_float):
    """
    Formats innings pitched from a float to a standard string format.

    For example, 1.2 becomes "1.1" (1 inning, 1 out) and 1.66 becomes "2.0".

    Args:
        ip_float (float): The innings pitched as a float.
...
def calculate_hitting_stats(df, season=None):
    """
    Calculates hitting statistics for a given DataFrame of plays.

    This function takes a DataFrame of game logs for a player or team and
    calculates a wide range of both standard and advanced hitting statistics.

    Args:
        df (pd.DataFrame): The DataFrame of game logs.
        season (str, optional): The season identifier (e.g., "S1"). Defaults to None.

    Returns:
        pd.Series or None: A Series containing the calculated hitting statistics,
    ...
    if use_old_results:
        ...
        pa_events = (
            ...
            | walks
            | strikeouts
            | {"FO", "PO", "LGO", "RGO", "LO", "DP", "TP", "Sac", "Bunt"}
        )
    else:
    ...
    num_gidp = pa_df[
        (pa_df["Old Result"].isin(["DP", "TP"])) | (pa_df["Exact Result"] == "BUNT DP")
    ].shape[0]
    ...
    if use_old_results:
        ...
        num_sf = pa_df[
            (pa_df["Exact Result"] == "FO")
            & (pd.to_numeric(pa_df["RBI"], errors="coerce").fillna(0) > 0)
        ].shape[0]
    ...
    obp = (
        (num_hits + num_walks) / (ab + num_walks + num_sf)
        if (ab + num_walks + num_sf) > 0
        else pd.NA
    )
    ...
    babip = (
        (num_hits - num_hr) / (ab - num_strikeouts - num_hr + num_sf)
        if (ab - num_strikeouts - num_hr + num_sf) > 0
        else pd.NA
    )
    sb_pct = num_sb / (num_sb + num_cs) if (num_sb + num_cs) > 0 else pd.NA
    ...
    if use_neutral_results:
        ...
        if not neutral_df.empty:
            ...
            if n_pa_current > 0:
                n_num_walks_current = n_pa_df[
                    n_pa_df[neutral_result_col].isin(n_walks)
                ].shape[0]
                n_num_hits_current = n_pa_df[
                    n_pa_df[neutral_result_col].isin(n_hits)
                ].shape[0]

                n_num_sf_current = n_pa_df[n_pa_df[neutral_result_col] == "Sac"].shape[
                    0
                ]
                n_num_sh_current = n_pa_df[
                    n_pa_df[neutral_result_col].isin(["Bunt", "BUNT Sac", "Bunt Sac"])
                ].shape[0]
                n_num_sacrifices_current = n_num_sh_current + n_num_sf_current

                n_ab_current = (
                    n_pa_current - n_num_walks_current - n_num_sacrifices_current
                )
                ...
                if (n_ab_current + n_num_walks_current + n_num_sf_current) > 0:
                    calculated_nOBP = (n_num_hits_current + n_num_walks_current) / (
                        n_ab_current + n_num_walks_current + n_num_sf_current
                    )
                    if not pd.isna(calculated_nOBP):
                        nOBP = calculated_nOBP

                if n_ab_current > 0:
                    n_num_doubles = n_pa_df[n_pa_df[neutral_result_col] == "2B"].shape[
                        0
                    ]
                    n_num_triples = n_pa_df[n_pa_df[neutral_result_col] == "3B"].shape[
                        0
                    ]
                    n_num_hr = n_pa_df[n_pa_df[neutral_result_col] == "HR"].shape[0]
                    n_num_singles = n_pa_df[
                        n_pa_df[neutral_result_col].isin(["1B", "BUNT 1B", "Bunt 1B"])
                    ].shape[0]
                    n_num_tb_current = (
                        n_num_singles
                        + (n_num_doubles * 2)
    ...
    batting_type = (
        df["Hitter Batting Type"].iloc[0]
        if "Hitter Batting Type" in df.columns
        and not df["Hitter Batting Type"].dropna().empty
        else None
    )
...
def calculate_pitching_stats(df, season=None):
    """
    Calculates pitching statistics for a given DataFrame of plays.

    This function takes a DataFrame of game logs for a pitcher and calculates
    a wide range of both standard and advanced pitching statistics.
    ...
    Returns:
        pd.Series or None: A Series containing the calculated pitching statistics,
                           or None if the input DataFrame is empty.
    """
    if df.empty:
        return None
    ...
    sb_pct_against = (
        num_sb_allowed / (num_sb_allowed + num_cs_against)
        if (num_sb_allowed + num_cs_against) > 0
        else pd.NA
    )
    ...
    if use_old_results:
        ...
        pa_events_exact = (
            hits_allowed | walks_allowed | strikeouts | single_out_bip | {"BUNT DP"}
        )
    ...
    num_sf_allowed = sac_events_df[
        pd.to_numeric(sac_events_df["RBI"], errors="coerce").fillna(0) > 0
    ].shape[0]
    ...
    num_singles_allowed = (
        num_hits_allowed - num_doubles_allowed - num_triples_allowed - num_hr_allowed
    )
    ...
    slga = (
        (
            num_singles_allowed
            + 2 * num_doubles_allowed
            + 3 * num_triples_allowed
            + 4 * num_hr_allowed
        )
        / ab_against
        if ab_against > 0
    ...
    babip_against = (
        (num_hits_allowed - num_hr_allowed) / babip_denom if babip_denom > 0 else pd.NA
    )
    ...
    num_gidp_allowed = bf_df[
        (bf_df["Old Result"].isin(["DP", "TP"])) | (bf_df["Exact Result"] == "BUNT DP")
    ].shape[0]
    num_gb_outs_allowed = (
        bf_df[bf_df[result_col].isin(ground_ball_events)].shape[0] + num_gidp_allowed
    )

    total_fb_gb_allowed = num_fb_outs_allowed + num_gb_outs_allowed
    gb_pct_against = (
        num_gb_outs_allowed / total_fb_gb_allowed if total_fb_gb_allowed > 0 else pd.NA
    )
    fb_pct_against = (
        num_fb_outs_allowed / total_fb_gb_allowed if total_fb_gb_allowed > 0 else pd.NA
    )
    ...
    gb_fb_ratio_against = (
        num_gb_outs_allowed / num_fb_outs_allowed if num_fb_outs_allowed > 0 else pd.NA
    )
    ...
    pitching_type = (
        df["Pitcher Pitching Type"].iloc[0]
        if "Pitcher Pitching Type" in df.columns
        and not df["Pitcher Pitching Type"].dropna().empty
        else None
    )
    ...
    return pd.Series(
        {
            ...
            "R": runs_allowed,
            "ER": earned_runs,
            "BB": num_walks_allowed,
            "IBB": num_ibb,
            "Auto BB": num_auto_bb_allowed,
            "K": num_strikeouts,
            "HR": num_hr_allowed,
            "1B": num_singles_allowed,
            "RGO": num_rgo_allowed,
            "LGO": num_lgo_allowed,
            "FO": num_fo_allowed,
            "PO": num_po_allowed,
            ...
            "SLGA": slga,
            "OPSA": opsa,
            "BABIP_A": babip_against,
            "HR%_A": hr_pct_against,
            "K%_A": k_pct_against,
            "BB%_A": bb_pct_against,
            "GB%_A": gb_pct_against,
            "FB%_A": fb_pct_against,
            "GB/FB_A": gb_fb_ratio_against,
            "Avg Diff": avg_diff,
            "RE24": re24,
            "WPA": wpa,
            "AB_A": ab_against,
            "SF_A": num_sf_allowed,
            "SH_A": num_sh_allowed,
            "2B_A": num_doubles_allowed,
            "3B_A": num_triples_allowed,
            "GB_outs_A": num_gb_outs_allowed,
            "FB_outs_A": num_fb_outs_allowed,
            "SB_A": num_sb_allowed,
            "CS_A": num_cs_against,
            "SB%_A": sb_pct_against,
            "Type": pitching_type,
        }
    )
...
def calculate_team_hitting_stats(df, league_stats_for_season):
    """
    Aggregates individual player hitting stats to calculate team hitting stats.

    Args:
        df (pd.DataFrame): DataFrame containing hitting stats for all players on a team.
        league_stats_for_season (dict): Dictionary with league-wide stats for the season.

    Returns:
        pd.Series: A Series containing the aggregated team hitting statistics.
    """
    sum_cols = [
        "G",
        "PA",
    ...
    if (
        league_stats_for_season
        and league_stats_for_season.get("lg_nOBP", 0) > 0
        and league_stats_for_season.get("lg_nSLG", 0) > 0
    ):
        ops_plus = 100 * (
            (team_nOBP / league_stats_for_season["lg_nOBP"])
            + (team_nSLG / league_stats_for_season["lg_nSLG"])
            - 1
        )
    else:
        ops_plus = 100
...
def calculate_team_pitching_stats(
    df, league_n_era_for_season, team_n_era, fip_constant
):
    """
    Aggregates individual pitcher stats to calculate team pitching stats.

    Args:
        df (pd.DataFrame): DataFrame containing pitching stats for all pitchers on a team.
        league_n_era_for_season (float): The league's neutral ERA for the season.
        team_n_era (float): The team's neutral ERA.
        fip_constant (float): The FIP constant for the season.

    Returns:
        pd.Series: A Series containing the aggregated team pitching statistics.
    """
    summed_stats = df[
        [
            "IP",
    ...
    obpa = (
        (num_hits_allowed + num_walks_allowed) / summed_stats["BF"]
        if summed_stats["BF"] > 0
        else 0
    )
    tb_allowed = (
        summed_stats["1B"]
    ...
    babip_denom = ab_against - num_strikeouts - num_hr_allowed + num_sf_allowed
    babip_against = (
        (num_hits_allowed - num_hr_allowed) / babip_denom if babip_denom > 0 else 0
    )
    hr_pct_against = (
        num_hr_allowed / summed_stats["BF"] if summed_stats["BF"] > 0 else 0
    )
    k_pct_against = num_strikeouts / summed_stats["BF"] if summed_stats["BF"] > 0 else 0
    bb_pct_against = (
        num_walks_allowed / summed_stats["BF"] if summed_stats["BF"] > 0 else 0
    )
    total_fb_gb_allowed = num_fb_outs_allowed + num_gb_outs_allowed
    gb_pct_against = (
        num_gb_outs_allowed / total_fb_gb_allowed if total_fb_gb_allowed > 0 else 0
    )
    fb_pct_against = (
        num_fb_outs_allowed / total_fb_gb_allowed if total_fb_gb_allowed > 0 else 0
    )
    gb_fb_ratio_against = (
        num_gb_outs_allowed / num_fb_outs_allowed if num_fb_outs_allowed > 0 else 0
    )
    sb_pct_against = (
        num_sb_allowed / (num_sb_allowed + num_cs_against)
        if (num_sb_allowed + num_cs_against) > 0
    ...
    fip = (
        ((13 * summed_stats["HR"]) + (3 * summed_stats["BB"]) - (2 * summed_stats["K"]))
        / ip
        + fip_constant
        if ip > 0
        else 0
    ...
    team_stats["W-L%"] = (
        summed_stats["W"] / (summed_stats["W"] + summed_stats["L"])
        if (summed_stats["W"] + summed_stats["L"]) > 0
        else 0
    )
    return team_stats


def calculate_career_hitting_stats(
    df, league_stats_by_season, include_type_column=True
):
    """
    Aggregates seasonal hitting stats to calculate career hitting stats for a player.

    Args:
        df (pd.DataFrame): DataFrame containing seasonal hitting stats for a player.
        league_stats_by_season (dict): Dictionary with league-wide stats for each season.
        include_type_column (bool): Whether to include the player 'Type' in the output.

    Returns:
        pd.Series: A Series containing the aggregated career hitting statistics.
    """
    sum_cols = [
        "G",
        "PA",
    ...
    lg_obp_series = df["Season"].map(
        lambda s: league_stats_by_season.get(s, {}).get("lg_nOBP")
    )
    lg_slg_series = df["Season"].map(
        lambda s: league_stats_by_season.get(s, {}).get("lg_nSLG")
    )
    ...
    if career_lg_obp > 0 and career_lg_slg > 0:
        ops_plus = 100 * (
            (career_nOBP / career_lg_obp) + (career_nSLG / career_lg_slg) - 1
        )
    else:
        ops_plus = 100
    ops_plus = round(ops_plus)
...
def calculate_career_pitching_stats(
    df, league_n_era_by_season, include_type_column=True
):
    """
    Aggregates seasonal pitching stats to calculate career pitching stats for a player.

    Args:
        df (pd.DataFrame): DataFrame containing seasonal pitching stats for a player.
        league_n_era_by_season (dict): Dictionary with league-wide neutral ERA for each season.
        include_type_column (bool): Whether to include the player 'Type' in the output.

    Returns:
        pd.Series: A Series containing the aggregated career pitching statistics.
    """
...
def get_base_state_svg(obc):
    """
    Generates an SVG image of a baseball diamond representing the runner state.

    Args:
        obc (int): The on-base code (a bitmask representing runners on base).

    Returns:
        str: A string containing the SVG markup.
    """
...
def generate_re_matrix_html(season_num):
    """
    Generates an HTML table for the Run Expectancy matrix of a given season.

    Args:
        season_num (int): The season number.

    Returns:
        dict or None: A dictionary containing the title and HTML content of the RE matrix,
    ...
    cache_path = os.path.join(
        script_dir, "..", "data", "cache", f"re_matrix_S{season_num}.csv"
    )
...
def _get_simulated_runs_for_inning(inning_df):
    """
    Simulates an inning play-by-play to determine runs scored.

    This function processes a DataFrame of plays for a single inning and
    simulates the outcome of each play based on the game's rulebook logic
    to calculate the number of runs scored on each play.

    Args:
        inning_df (pd.DataFrame): DataFrame of plays for a single inning.

    Returns:
        pd.Series: A Series with the number of runs scored on each play.
    """
    if inning_df.empty:
    ...
    for _, play in inning_df.iterrows():
        ...
        if result == "HR":
            ...
            if runners[2]:
                runs_this_play += 1  # Runner from 3rd scores
            if runners[1]:
                runs_this_play += 1  # Runner from 2nd scores
            ...
            if (
                ...
                if outs <= 2:
                    if runners[2]:
                        runs_this_play += 1
                        runners[2] = 0
                    if runners[1] and not (is_lgo and not runners[0]):
                        runners[2] = 1
                        runners[1] = 0
        elif result == "DP":
...
def get_run_expectancy_matrix(season, season_df, is_most_recent_season=False, cache_prefix=""):
    ...
    Run expectancy is the average number of runs scored from a given base-out state
    ...
    for col in ["OBC", "Outs", "Run"]:
        season_df[col] = (
            pd.to_numeric(season_df[col], errors="coerce").fillna(0).astype(int)
        )

    # Get simulated runs for all innings
    simulated_runs = season_df.groupby("Inning ID").apply(
        _get_simulated_runs_for_inning, include_groups=False
    )
    ...
    re_matrix_df = (
        all_plays_df.groupby(["OBC", "Outs"])["RunsAfter"].mean().reset_index()
    )
...
def _simulate_neutral_inning(inning_df, re_matrix):
    """
    Simulates a single inning based on 'Result at Neutral' to calculate neutral runs and outs.

    This function is used to determine a pitcher's responsibility for runs scored,
    ignoring the impact of inherited runners.

    Args:
        inning_df (pd.DataFrame): DataFrame of plays for a single inning.
        re_matrix (dict): The run expectancy matrix for the season.

    Returns:
        pd.Series: A Series containing the neutral runs and outs for the inning.
    """
    obc_to_runners_map = {
    ...
    def runners_to_obc(runners_dict, inherited_only=False):
        obc = 0
        if runners_dict.get(1) and (not inherited_only or runners_dict[1] == "i"):
            obc |= 1
        if runners_dict.get(2) and (not inherited_only or runners_dict[2] == "i"):
            obc |= 2
        if runners_dict.get(3) and (not inherited_only or runners_dict[3] == "i"):
            obc |= 4
        return obc
    ...
    for _, play in inning_df.iterrows():
        ...
        if result == "HR":
            ...
            if runners.get(1) and runners.get(2):
                new_runners[3] = runners[2]
            if runners.get(1):
                new_runners[2] = runners[1]
...
def calculate_neutral_pitching_stats(df, re_matrix):
    ...
    This function groups a pitcher's appearances by inning and applies the
    neutral inning simulation to each one, summing the results to get a
    season-level total for neutral runs and outs.

    Args:
        df (pd.DataFrame): DataFrame of a pitcher's plays for a season.
        re_matrix (dict): The run expectancy matrix for the season.

    Returns:
        pd.Series: A Series containing the total neutral runs and outs.
    """
    if df.empty or not re_matrix:
        return pd.Series({"nRuns": 0, "nOuts": 0})

    # This will be slow, but it's the only way to do it row-by-row
    inning_stats = df.groupby("Inning ID").apply(
        lambda x: _simulate_neutral_inning(x, re_matrix), include_groups=False
    )
...
def calculate_games_started(df):
    ...
    for i, ((season, game_id), game_df) in enumerate(game_groups):
        ...
        pitchers_A = (
            game_df[game_df["Pitcher Team"] == team_A]["Pitcher ID"].unique().tolist()
        )
        pitchers_B = (
            game_df[game_df["Pitcher Team"] == team_B]["Pitcher ID"].unique().tolist()
        )

        if pitchers_A:
            starter_id_A = game_df[game_df["Pitcher Team"] == team_A][
                "Pitcher ID"
            ].iloc[0]
            achievements.append(
                {
                    "Season": season,
                    "Pitcher ID": starter_id_A,
                    "Stat": "GS",
                    "Team": team_A,
                }
            )
        ...
        if pitchers_B:
            starter_id_B = game_df[game_df["Pitcher Team"] == team_B][
                "Pitcher ID"
            ].iloc[0]
            achievements.append(
                {
                    "Season": season,
                    "Pitcher ID": starter_id_B,
                    "Stat": "GS",
                    "Team": team_B,
                }
            )
    ...
    if not achievements:
        return pd.DataFrame(columns=["Season", "Pitcher ID", "Team", "GS"])

    achievements_df = pd.DataFrame(achievements)
    agg_df = (
        achievements_df.groupby(["Season", "Pitcher ID", "Team", "Stat"])
        .size()
...
def calculate_game_achievements(df):
    ...
    for i, ((season, game_id), game_df) in enumerate(game_groups):
        ...
        if pitchers_A:
            finisher_id_A = game_df[game_df["Pitcher Team"] == team_A][
                "Pitcher ID"
            ].iloc[-1]
            achievements.append(
                {
                    "Season": season,
                    "Pitcher ID": finisher_id_A,
                    "Stat": "GF",
                    "Team": team_A,
                }
            )
        ...
        if pitchers_B:
            finisher_id_B = game_df[game_df["Pitcher Team"] == team_B][
                "Pitcher ID"
            ].iloc[-1]
            achievements.append(
                {
                    "Season": season,
                    "Pitcher ID": finisher_id_B,
                    "Stat": "GF",
                    "Team": team_B,
                }
            )
    ...
    if not achievements:
        return pd.DataFrame(columns=["Season", "Pitcher ID", "Team", "GF", "CG", "SHO"])
...
def preprocess_gamelogs_for_stat_corrections(df, player_id_to_name_map):
    ...
    for game_id, game_df in df.groupby(["Season", "Game ID"]):
        ...
        for index, play in game_df.iterrows():
            ...
            if is_steal_event and not is_batter_on_base:
                ...
                if original_runner_id:
                    ...
                    original_runner_pa_mask = (
                        ...
                        & (
                            df["Old Result"].isin(on_base_events)
                            | df["Exact Result"].isin(on_base_events)
                        )
                    )
            ...
            if result in multi_steal_events:
                ...
                for base, runner_id in current_runners.items():
                    if runner_id and runner_id != caught_runner:
                        ...
                        new_row["Hitter"] = player_id_to_name_map.get(
                            runner_id, "Unknown Player"
                        )
                        # Determine the type of steal based on the base they are advancing to
                        new_row["Exact Result"] = (
                            f"STEAL {base + 1}B" if base < 3 else "STEAL Home"
                        )
            ...
            if play["Old Result"] in {"1B", "2B", "3B", "HR", "BB", "IBB"} or play[
                "Exact Result"
            ] in {"BUNT 1B", "Bunt 1B"}:
                if play["Old Result"] == "1B" or play["Exact Result"] in {
                    "BUNT 1B",
                    "Bunt 1B",
                    "BB",
                    "IBB",
...
def aggregate_decisions(df, games_df):
    if df.empty:
        return pd.DataFrame(
            columns=["Season", "Pitcher ID", "Team", "W", "L", "SV", "HLD"]
        )

    base_df = (
        games_df[["Season", "Pitcher ID", "Pitcher Team"]]
        .drop_duplicates()
        .rename(columns={"Pitcher Team": "Team"})
    )
    pitcher_teams_in_game = (
        games_df[["Season", "Game ID", "Pitcher ID", "Pitcher Team"]]
        .drop_duplicates()
        .rename(columns={"Pitcher Team": "Team"})
    )

    def get_agg_stat(decision_col, stat_name):
        stat_df = df[df[decision_col].notna()][
            ["Season", "Game ID", decision_col]
        ].rename(columns={decision_col: "Pitcher ID"})
        stat_with_teams = stat_df.merge(
            pitcher_teams_in_game, on=["Season", "Game ID", "Pitcher ID"], how="left"
        )
        return (
            stat_with_teams.groupby(["Season", "Pitcher ID", "Team"])
            .size()
    ...
    if "holds" in df.columns and not df.explode("holds").dropna(subset=["holds"]).empty:
        ...
        holds_df = holds_exploded[["Season", "Game ID", "holds"]].rename(
            columns={"holds": "Pitcher ID"}
        )
        holds_with_teams = holds_df.merge(
            pitcher_teams_in_game, on=["Season", "Game ID", "Pitcher ID"], how="left"
        )
        holds_agg = (
            holds_with_teams.groupby(["Season", "Pitcher ID", "Team"])
            .size()
            .reset_index(name="HLD")
        )
    else:
    ...
    for stat_df in [wins_agg, losses_agg, saves_agg, holds_agg]:
        if not stat_df.empty:
            stat_df["Pitcher ID"] = (
                pd.to_numeric(stat_df["Pitcher ID"], errors="coerce")
                .dropna()
                .astype(int)
            )
            agg_df = agg_df.merge(
                stat_df, on=["Season", "Pitcher ID", "Team"], how="left"
            )
...
def generate_league_data(
        ...
        for season in sorted_seasons_keys:
            ...
            df.rename(
                columns={
                    ...
                    "Batting Type": "batting_type",
                    "Pitching Type": "pitching_type",
    ...
    pitchers_with_ids = combined_df[combined_df["Pitcher ID"].notna()][
        ["Pitcher ID", "Pitcher", "Season"]
    ].rename(columns={"Pitcher ID": "Player ID", "Pitcher": "Player Name"})
    players_with_ids = pd.concat(
        [hitters_with_ids, pitchers_with_ids]
    ).drop_duplicates()
    ...
    if no_id_mask.any():
        combined_df.loc[no_id_mask, "Pitcher ID"] = combined_df.loc[
            no_id_mask, "Pitcher"
        ].apply(get_or_assign_temp_id)

    no_id_mask_hitter = combined_df["Hitter ID"].isna()
    if no_id_mask_hitter.any():
        combined_df.loc[no_id_mask_hitter, "Hitter ID"] = combined_df.loc[
            no_id_mask_hitter, "Hitter"
        ].apply(get_or_assign_temp_id)
    ...
    if most_recent_season:
        ...
        if not most_recent_season_df.empty and not is_fcb:
            ...
            decision_games_mask = ~(
                (combined_df["Season"] == most_recent_season)
                & (combined_df["Session"] == max_session)
            )
    ...
    pitching_decisions_df = pitching_decisions_df.merge(
        game_types, on=["Season", "Game ID"], how="left"
    )
    regular_season_decisions = pitching_decisions_df[
        pitching_decisions_df["GameType"] == "Regular"
    ]
    ...
    all_players = pd.concat(
        [
            combined_df[["Hitter ID", "Hitter", "Season", "Session"]].rename(
                columns={"Hitter ID": "Player ID", "Hitter": "Player Name"}
            ),
            combined_df[["Pitcher ID", "Pitcher", "Season", "Session"]].rename(
                columns={"Pitcher ID": "Player ID", "Pitcher": "Player Name"}
            ),
        ]
    )
    ...
    all_players.sort_values(
        by=["Season_num", "Session"], ascending=[False, False], inplace=True
    )

    player_names = (
        all_players.groupby("Player ID")["Player Name"]
    ...
    if "Batter WPA" in combined_df.columns:
        combined_df["Batter WPA"] = (
            pd.to_numeric(
                combined_df["Batter WPA"].astype(str).str.strip("%"), errors="coerce"
            ).fillna(0)
            / 100
        )
    if "Pitcher WPA" in combined_df.columns:
        combined_df["Pitcher WPA"] = (
            pd.to_numeric(
                combined_df["Pitcher WPA"].astype(str).str.strip("%"), errors="coerce"
            ).fillna(0)
            / 100
        )
    ...
    combined_df.sort_values(
        by=["Season_num", "Session"], ascending=[True, True], inplace=True
    )
    ...
    if all_season_names.size > 0:
        most_recent_season_num = max(
            [int(s.replace("S", "")) for s in all_season_names]
        )
    ...
    for season in sorted_seasons:
        ...
        season_df["OBC"] = (
            pd.to_numeric(season_df["OBC"], errors="coerce").fillna(0).astype(int)
        )
        season_df["Outs"] = (
            pd.to_numeric(season_df["Outs"], errors="coerce").fillna(0).astype(int)
        )
        re_before = season_df.apply(
            lambda row: re_matrix.get((row["OBC"], row["Outs"]), 0), axis=1
        )
