...
def _add_war_columns(gamelog_df):
    ...
    for season, season_df in gamelog_df.groupby('Season'): # iterate through seasons
        ...
        total_pa = len(season_df[~season_df['PA Type'].isin([0, 15, 32])]) # number of plate apperances in the season. PA in this case also includes stolen base attempts. 32 (LLR only) is a standalone error event with no batter, not a real PA.
        ...
        for idx, row in season_df.iterrows(): # iterate through each play
            if row['PA Type'] in [0, 15, 32]: # ignore Manfred runners, trailing runners on multisteals, and standalone error events (LLR only, no batter)
                batting_rar = 0
                pitching_rar = 0
